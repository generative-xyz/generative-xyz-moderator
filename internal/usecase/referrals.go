...
func (u Usecase) CreateReferral( referrerID string, referreeID string) error {
	...
	logger.AtLog.Logger.Info("CreateReferral", zap.Any("referrerID", referrerID), zap.Any("referreeID", zap.Any("referreeID)", referreeID)))
	...
	logger.AtLog.Logger.Info("CreateReferral", zap.Any("InsertReferral", zap.Any("inserted)", inserted)))
...
func (u Usecase) GetReferrals( req structure.FilterReferrals) (*entity.Pagination, error) {
	logger.AtLog.Logger.Info("GetReferrals", zap.Any("req", zap.Any("req)", req)))
	...
	if err != nil {
		logger.AtLog.Logger.Error("u.Repo.FilterReferrals", zap.Error(err))
		return nil, err
	}
	...
	logger.AtLog.Logger.Info("GetReferrals", zap.Any("req", req), zap.Any("referrals",referrals), zap.Any("referrals", zap.Any("referrals.Total)", referrals.Total)))

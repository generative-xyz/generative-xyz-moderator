...
func (u Usecase) GetChartDataERC20ForGMCollection(newcity entity.NewCityGm, transferedETH []string, ens string, avatar string) (*structure.AnalyticsProjectDeposit, error) {
	...
	if err == nil {
		...
		if err == nil {
			logger.AtLog.Logger.Error("GetChartDataERC20ForGMCollection cached", zap.Error(err), zap.String("gmAddress", newcity.Address), zap.String("tcAddress", newcity.Address), zap.Bool("cached", true))
			return result, nil
		}
	}
	...
	if err != nil {
		logger.AtLog.Logger.Error("GetChartDataERC20ForGMCollection err1111", zap.Error(err), zap.String("gmAddress", newcity.Address), zap.String("tcAddress", newcity.Address))
		return nil, err
	}

	logger.AtLog.Logger.Info("GetChartDataEthForGMCollection", zap.Any("moralisERC20BL", moralisERC20BL), zap.String("tcAddress", newcity.Address), zap.String("gmAddress", newcity.Address), zap.String("key", key))
	...
	if len(items) > 0 {
		...
		logger.AtLog.Logger.Info("GetChartDataEthForGMCollection len(items) > 0", zap.Any("moralisERC20BL", moralisERC20BL), zap.String("tcAddress", newcity.Address), zap.String("gmAddress", newcity.Address), zap.String("key", key), zap.Any("resp", resp))
		...
		if time.Now().Add(time.Hour * -12).After(*newcity.UpdatedAt) {
			...
			logger.AtLog.Logger.Info("GetChartDataEthForGMCollection len(items) = 0", zap.Any("moralisERC20BL", moralisERC20BL), zap.String("tcAddress", newcity.Address), zap.String("gmAddress", newcity.Address), zap.String("key", key), zap.Any("resp", resp), zap.Bool("after12Hours", true))
		}
	}
...
func (u Usecase) GetChartDataEthForGMCollection(tcAddress string, gmAddress string, transferedETH []string, oldData bool, ens string, avatar string) (*structure.AnalyticsProjectDeposit, error) {
	...
	if err == nil {
		err = json.Unmarshal([]byte(*cached), result)
		if err == nil {
			logger.AtLog.Logger.Info("GetChartDataEthForGMCollection cached", zap.Any("result", result), zap.String("gmAddress", gmAddress), zap.String("tcAddress", tcAddress), zap.Bool("cached", true))
			return result, nil
		}
	}
	...
	if ethRate == 0 {
		...
		if err != nil {
			logger.AtLog.Logger.Error("GetChartDataEthForGMCollection", zap.Error(err), zap.String("gmAddress", gmAddress), zap.String("tcAddress", tcAddress))
			return nil, err
		}
	...
	//time.Sleep(time.Millisecond * 250)
	if err != nil {
		logger.AtLog.Logger.Error("GetChartDataEthForGMCollection err2222", zap.Error(err), zap.String("gmAddress", gmAddress), zap.String("tcAddress", tcAddress))
		//return nil, err
		moralisEthBL = new(nfts.MoralisBalanceResp)
		temp, err := u.EtherscanService.AddressBalance(gmAddress)
		if err != nil {
			logger.AtLog.Logger.Error("GetChartDataEthForGMCollection err3333", zap.Error(err), zap.String("gmAddress", gmAddress), zap.String("tcAddress", tcAddress))
			return nil, err
		}
		moralisEthBL.Balance = temp.Result
		if moralisEthBL.Balance == "" {
			logger.AtLog.Logger.Error("GetChartDataEthForGMCollection err4444", zap.Error(err), zap.String("gmAddress", gmAddress), zap.String("tcAddress", tcAddress))
			return nil, err
		}
	}
	...
	logger.AtLog.Logger.Info("GetChartDataEthForGMCollection", zap.Any("moralisEthBL", moralisEthBL), zap.Bool("oldData", oldData), zap.String("gmAddress", gmAddress), zap.String("tcAddress", tcAddress), zap.String("key", key))
	...
	if totalEth > 0 {
		...
		logger.AtLog.Logger.Info("GetChartDataEthForGMCollection totalEth > 0", zap.String("tcAddress", tcAddress), zap.String("gmAddress", gmAddress), zap.String("key", key), zap.Any("resp", resp))

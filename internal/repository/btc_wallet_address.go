...
func (r Repository) VolumeByProjectIDs(projectIDs []string, amountType string) (*AggregateWalletAddres, error) {
	...
	pipeLine := bson.A{
		bson.M{"$match": bson.M{"$and": bson.A{
			...
			bson.M{"mintResponse.issent": true},
		}}},
		bson.M{"$group": bson.M{"_id": "$projectID", "amount": bson.M{"$sum": bson.M{"$toDouble": "$amount"}}}},
		bson.M{"$sort": bson.M{"_id": -1}},
	}
	
	cursor, err := r.DB.Collection(amountType).Aggregate(context.TODO(), pipeLine, nil)
	if err != nil {
		return nil, err
	}
